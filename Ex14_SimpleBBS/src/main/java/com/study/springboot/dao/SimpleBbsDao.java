...
public class SimpleBbsDao implements ISimpleBbsDao 
{
	/*
		...
		오버라이딩 해야 한다. 다라서 이부분은 자동완성 기능을 사용하면 
		편리하다.
	*/
	...
	public SimpleBbsDto viewDao(String id) {
		...
		/*
			...
			메서드를 사용한다. 두번재 인자를 통해 인출한 레코드를 DTO에 자동으로
			저장해준다. 세번째인자를 통해 쿼리문의 인파리머터를 설정 한다. 이때
	...
	public int writeDao(final String writer, final String title, final String content) {
		...
		/*
			insert, delete, update와 같이 행의 변화가 생기는 쿼리문을 실행할 때는
			update() 메서드를 사용한다. 실행결과로 변환된 행의 갯수 즉 입력된 행의 갯수
			가 변환되게 된다. 
		*/
		// 인파리미터가 있는 쿼리문 작성
		String query = 
